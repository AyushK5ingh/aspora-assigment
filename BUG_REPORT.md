...
- **Root Cause**: The React `ToastItem` component mounted into the DOM with the `.toast--visible` CSS class already attached immediately. Because CSS `transition` algorithms only trigger when a property changes *after* the element is painted to the screen, applying the final state during the initial mount bypassed the animation engine completely.
